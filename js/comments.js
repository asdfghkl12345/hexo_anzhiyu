(()=>{let e={API_URL:"https://twikoo.ruom.top",ADMIN_EMAIL_MD5:"f2c9c64c90a00afeed5ba410e5447a0d01aa294874bd662032a27c5385bcde1c",PAGE_SIZE:5,LOADING_GIF:"https://lib.bsgun.cn/Hexo-static/img/loading.gif",async fetchComments(){let t=new AbortController;var e=setTimeout(()=>t.abort(),5e3);try{var a=(await(await fetch(this.API_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({event:"GET_RECENT_COMMENTS",includeReply:!0,pageSize:this.PAGE_SIZE}),signal:t.signal})).json()).data;return a}catch(t){return console.error("获取评论出错:",t),null}finally{clearTimeout(e)}},formatTimeAgo(t){var e=Math.floor((Date.now()-new Date(t))/1e3);return e<60?"刚刚":e<3600?Math.floor(e/60)+"分钟前":e<86400?Math.floor(e/3600)+"小时前":e<604800?Math.floor(e/86400)+"天前":new Date(t).toLocaleDateString("zh-CN",{month:"numeric",day:"numeric"})+"日"},formatContent(t){return t?t.replace(/<pre><code>[\s\S]*?<\/code><\/pre>/g,"[代码块]").replace(/<code>([^<]{4,})<\/code>/g,"[代码]").replace(/<code>([^<]{1,3})<\/code>/g,"$1").replace(/<img[^>]*>/g,"[图片]").replace(/<a[^>]*?>[\s\S]*?<\/a>/g,"[链接]").replace(/<[^>]+>/g,"").replace(/&(gt|lt|amp|quot|#39|nbsp);/g,t=>({">":">","<":"<","&":"&",quot:'"',"#39":"'",nbsp:" "})[t.slice(1,-1)]).replace(/\s+/g," ").trim():""},generateCommentHTML(t){var{created:t,comment:e,url:a,avatar:i,nick:n,mailMd5:o,id:s}=t,t=this.formatTimeAgo(t),e=this.formatContent(e);return`
          <div class="aside-list-item" title="${e}" onclick="pjax.loadUrl('${a}#${s}')">
            <div class="thumbnail">
              <img class="aside-list-avatar" src="${i}" alt="avatar">
            </div>
            <div class="content">
              <div class="aside-list-author">
                ${n} ${o===this.ADMIN_EMAIL_MD5?`
        ...
        `},async insertComponent(){let e=document.getElementById("latest-comments");if(e){e.innerHTML=`<img src="${this.LOADING_GIF}" style="display: flex;min-height: 346px;object-fit: cover;">`;var a=await this.fetchComments();let t;t=null===a?this.getErrorTemplate("exclamation-circle","评论加载失败，请稍后再试"):0===a.length?this.getErrorTemplate("comment-slash","还没有评论呢~ 快来抢沙发吧！"):a.map(this.generateCommentHTML.bind(this)).join(""),e.style.opacity="0",e.innerHTML=t,requestAnimationFrame(()=>{e.style.transition="opacity 0.3s ease-in",e.style.opacity="1"})}}};["DOMContentLoaded","pjax:success"].forEach(t=>document.addEventListener(t,()=>{var t;(t=document.createElement("style")).textContent=`
